...
import ReactMarkdown from 'react-markdown'; // ReactMarkdown : 
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center p-10 gap-8 font-sans">
      ...
      <h1 className="text-5xl font-extrabold bg-gradient-to-r from-blue-400 via-cyan-400 to-teal-300 bg-clip-text text-transparent tracking-tight">
        Agent v4.0
      </h1>
      ...
      {(response || isLoading) && (
        <div className="w-full max-w-3xl bg-slate-900/80 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur-sm relative min-h-[200px]">
          ...
          {isLoading && !response ? (
             <div className="animate-pulse flex space-x-4"><div className="flex-1 space-y-4 py-1"><div className="h-4 bg-slate-700 rounded w-3/4"></div><div className="h-4 bg-slate-700 rounded"></div></div></div>
          ) : (
